...
cam_methods=( "shapleycam" "shapleycam_mean"  "gradcam"  "gradcamplusplus" "layercam" "gradcamelementwise" "randomcam")
...
for model in "${models[@]}"; do
  for cam_method in "${cam_methods[@]}"; do
    python usage_examples/ADCC_imagenet_gist.py --model "$model" --cam-method "$cam_method" --batch-size "${batch_sizes[$model]}"  --output-file gist_avgpoolpre2.txt
