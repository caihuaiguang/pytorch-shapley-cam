...
cam_methods=("shapleycam" "shapleycam_mean" "shapleycam_hires" "gradcam"  "gradcamelementwise" "hirescam" "gradcamplusplus"  "xgradcam" "layercam" "randomcam")
...
for model in "${models[@]}"; do
  for cam_method in "${cam_methods[@]}"; do
    python usage_examples/ADCC_imagenet_conv.py --model "$model" --cam-method "$cam_method" --batch-size "${batch_sizes[$model]}"  --output-file output_ADCC_final.txt
